...
```{r lm-summary-results, echo=FALSE}
summary(reg)
```
...
modelsummary(
    ...
    notes = list("This table presents results of univariate and multivarate 
                 regression models."),
    output = "./output/table1.png" # can change to other formats
    ) 
...
# Data Visualization `r emo::ji("sparkles")`
...
To export your plots, you can use the command `ggsave()`. If you do not save your plot as an object, `ggsave()` will automatically save whatever plot is currently in your plot viewer (which can lead to mistakes if you're testing out a lot of plots). If you have saved your plot as an object, then you need to specify the object in your call to `ggsave()`.

```{r ggsave, eval = F}
top10 + bottom10 # print the plot first, so it's in the plot viewer
ggsave(filename = "./output/gdp.png") # can specify other filetypes

ggsave(plot = top10, # specifies the plot stored as "top10" in the environment
       filename = "./output/gdp_top10.png")

ggsave(plot = bottom10,  # specifies the plot stored as "bottom10" in the environment
       filename = "./output/gdp_bottom10.png")
...
There is a companion R script that has all the commands I used in today's workshop. 
